    ...
    try:
        while True:
            try:
                ...
                # Handle the error from read_loop
                print(f"Error reading from input device: {e}")
                await send_telegram_message(f"Error reading from input device: {e}")
                print("Attempting to reconnect to input device...")
                time.sleep(5)  # Wait before trying to reconnect
                try:
                    # Re-initialize the input device
                    dev.close()
                    dev = find_input_device()
                    print("Reconnected to input device")
                    await send_telegram_message("Reconnected to input device")
                except Exception as recon_err:
                    print(f"Failed to reconnect: {recon_err}")
                    await send_telegram_message(f"Failed to reconnect to input device: {recon_err}")
